tell application "Tunnelblick"
    ...
    if vpnName = ""
        log "Set the environment variable PRIMARY_TUNNELBLICK_VPN_NAME to the name of the network you want to disconnect from"
        error number -128
    end if
    ...
    if tbstate is not "SLEEP" and tbstate is not "EXITING" then
        ...
        repeat until result = "SLEEP" or result = "EXITING"
            log (vpnName & " is in state [" & result & "]. Waiting...")
            delay 1
            get state of first configuration where name = vpnName
        end repeat
    end if

    log (vpnName & " is now disconnected.")
end tell
